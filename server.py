...
app = Flask(__name__)
api = Api(app=app)
ns_type = api.namespace(
    "b-leader-classification",
    description="Classify the leader types of HLA-B allotype(s) and genotype(s).",
)
ns_match = api.namespace(
    "b-leader-match",
    description="Classify the leader match status of a HLA-B genotype against HLA-B genotype(s).",
)

allele_model = api.model(
    "allele", {"allele": fields.String(example="B*07:02:01/B*07:02:02", required=True)}
)
...
class LeaderAllotype(Resource):
    def get(self, hla_allotype_input):
        ...
        try:
            hla_allotypes = [
                allo.strip() for allo in list(set(hla_allotype_input.split(",")))
            ]
            results = []
            mapper = LeaderMapper()
...
class LeaderGenotype(Resource):
    def get(self, hla_genotype_input):
        ...
        try:
            hla_genotypes = [
                geno.strip() for geno in list(set(hla_genotype_input.split(",")))
            ]
            results = []
            mapper = LeaderMapper()
...
class MatchStatus(Resource):
    ...
    model = api.model(
        "HLA-B Genotypes Input",
        {
            "hla-b_genotype_patient": fields.String(
                required=True,
                description="HLA-B genotype of the patient",
                help="HLA-B genotype format: B*XX:XX+B*YY:XX",
            ),
            "hla-b_genotype_donors": fields.String(
                required=True,
    ...
    def post(self):
        ...
        try:
            ...
            match = HlaBGenotypeMatch(
                HlaBGenotype(patient["genotype"], id=patient["id"]),
