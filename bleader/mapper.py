...
class LeaderMapper(object):
    def __init__(self) -> None:
        ...
        self.map = {
            HlaBAllotype(allotype): LeaderAllotype(leader_map[allotype])
            for allotype in leader_map
            if re.search("B\*\d+(\:\d+)*$", allotype)
        }
    ...
    def get_match_status_info(self, genotype_match: HlaBGenotypeMatch):
        ...
        if not isinstance(genotype_match, HlaBGenotypeMatch):
            raise InvalidInputError(
                genotype_match, "Did not provide an HlaBGenotypeMatch object."
            )
        ...
        for match in matches:
            ...
            shared_leader = (
                shared_allotype_patient
            ) = shared_allotype_donor = leader_match_status = None
            ...
            if index_shared is not None and match_code[index_shared] in ["A", "P"]:
                shared_leader = (
                    leader_genotype_patient[index_shared]
                    == leader_genotype_donor[index_shared]
                    and leader_genotype_patient[index_shared]
                    or "X"
                )
                shared_allotype_patient = hlaB_genotype_patient["name"].split("+")[
                    index_shared
                ]
                shared_allotype_donor = hlaB_genotype_donor["name"].split("+")[
                    index_shared
                ]
                unshared_leader_patient = leader_genotype_patient[index_unshared]
                unshared_leader_donor = leader_genotype_donor[index_unshared]
                match_status = (
                    unshared_leader_patient == unshared_leader_donor
                    and "matched"
                    or "mismatched"
                )
                leader_match_status = (
                    unshared_leader_patient + unshared_leader_donor + shared_leader
                )
            results.append(
                {
                    "hlaB_genotype_match": match_code,
                    ...
                    "leader_match_status": (
                        match["match_code"] not in ["AA", "MM", "LL", "ML"]
                        and leader_match_status
                        or None
                    ),
                }
            )
    ...
    def _rank(self, results: list):
        ...
        leader_genotype_patient = results[0]["leader_genotype_patient"][
            "leader_genotype"
        ]
        leader_orders = {
            "MT": ["MMT", "TTM"],
        ...
        leader_order = (
            leader_genotype_patient in leader_orders
            and leader_orders[leader_genotype_patient]
            or []
        )
        leader_order += ["other"]
            ...
            for match_code in hla_order:
                ...
                for i, result in enumerate(results):
                    if match_code == result["hlaB_genotype_match"] and (
                        match_status == result["leader_match_status"]
                        or (
                            match_status == "other"
                            and result["leader_match_status"] not in leader_order
                        )
                    ):
                        result["rank"] = rank
                        results[i]["rank"] = rank
    ...
    def get_leader_genotype_info(self, inp_genotype: HlaBGenotype):
        ...
        return {
            "leader_genotype": str(allotype_one["common_leader"])
        ...
        for allotype in inp_allotypes:
            ...
            for hla_b in self.map:
                if hla_b.fields[0] == allotype.fields[0]:
                    ...
                    while (
                        match
                        and i < len(hla_b.fields)
                        and hla_b.fields[i]
                        and allotype.fields[i]
                    ):
            ...
            if not allotype_matches and allotype.name not in [
                str(hla_b) for hla_b in output_map.keys()
            ]:
                output_map[allotype] = "X"
            if len(allele_family) and all(
                [str(leader) == "X" for leader in output_map.values()]
            ):
    ...
    def _summarize_leaders(self, inp_allotype, output_map, family_leader):
        ...
        known_leaders = [
            str(leader) for leader in output_map.values() if str(leader) != "X"
        ]
        if known_leaders:
        ...
        for hla_b, leader in output_map.items():
            if str(leader) not in [common_leader, "X"]:
                ...
                known.append(hla_b)
        return {
            "hla-b_allotype": dict(inp_allotype),
